...
#### 7. Pandas 라이브러리에 대한 설명으로 옳지 않은 것을 고르세요. 2

1. 데이터 분석에 초점이 맞춰져 빅데이터를 다루는데 편리하다.
2. Series는 다차원 배열 자료구조이다. <1차원>
3. DataFrame는 각 값에 연결된 인덱스 값을 저장한다.
4.  DataFrame 객체 생성을 위해 딕셔너리가 사용된다. (사용할 수 있다,)



#### 8. 결과 창과 동일하게 출력하기 위해 물음표에 들어가야 할 코드를 고르세요. 2
...
4. Python idle 환경에서는 반드시 show()를 작성해야 시각화 결과를 확인할 수 있다.<idle 환경은 통합개발 환경경 을 말하는 거임>
...
#### 11. Seaborn 라이브러리에 대한 설명으로 옳지 않은 것을 고르세요. 3 

1. Matplotlib 기반으로 다양한 색상과 테마가 추가된 패키지이다.
2. 그래프 그리기 함수뿐 아니라 다양한 예제 데이터셋을 제공한다.
3. regplot()은 산점도와 히스토그램을 함께 그려주는 함수이다. <다른거 그려주는 거임>
...
4. 교차 검증 방법은 과소 적합을 피하기 위해서 사용한다. <성능을 조금이라도 올리기 위해?>
...
4. shape 함수를 이용해 데이터 타입을 확인할 수 있다. <크기와 모양 >



#### 16. 탐색적 데이터 분석 과정에 대한 설명으로 옳지 않은 것을 고르세요. 1
...
3. isnull 함수만으로 결측치의 개수를 확인할 수 있다. <sum() 함수와 같이 사용 해야 됨됨>
4. 데이터 수집 과정에서 발생할 수 있다. <충분히 발생생>



#### 18. 이상치에 대한 설명으로 옳지 않은 것을 고르세요. 4 

1. 다른 데이터 포인트와 크게 벗어난값을 의미한다.
2. 단순 삭제 방법을 통해 처리할 수 있다.
3. 이상치를 판단하는 기준은 여러 가지이다.
4. 이상치에 유의미한 정보가 있다고 판단되는 경우 주로 대체 방법을 사용한다.<유의미한 정보는 변수화 시켜라라>
...
#### 21. 탐색적 데이터 분석 과정에서 옳지 않은 설명을 고르세요. 2
