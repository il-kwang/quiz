...
def outlier_index(fare_amount, passenger_count, pickup_longitude, pickup_latitude, dropoff_longitude, dropoff_latitude):
    ...
    for i, x, y, _x, _y in zipped:
        # 타는 곳(pickup_longitude,pickup_latitude)과 내리는 곳(dropoff_longitude, dropoff_latitude)이 같은 데이터의 인덱스를 idx_zero_distance에 저장합니다.
        if (x == _x) and (y == _y):
...
def remove_outlier(dataframe, list_idx):
    # list_idx를 인덱스로 사용하여 DataFrame에서 해당 인덱스를 제거합니다.
